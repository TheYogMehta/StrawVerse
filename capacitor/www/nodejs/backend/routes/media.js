    ...
    try {
      ...
      if (LocalMalProvider === "local") {
        ...
        if (global.db && id) {
          ...
          if (!resolvedMalId) {
            try {
              const localRow = await global.db
                .prepare(
        ...
        if (resolvedMalId !== undefined) {
          ...
          if (global.mappingDb) {
            try {
              let providers = [];
              try {
                const rows = await global.mappingDb
                  .prepare(
              ...
              const query = `
                ...
                ${leftJoins}
              `;
              mappingRow = await global.mappingDb.prepare(query).get(...params);
            } catch (e) {}
          }
...
router.post("/api/mapping/link-item", async (req, res) => {
  try {
    ...
    const existing = await queryOne(
      `SELECT * FROM ${itemType} WHERE id = ? OR id = ?`,
      [finalId, oldId],
    );
