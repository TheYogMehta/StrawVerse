...
function getProviderKeyField(table) {
  try {
    if (global.mappingDb) {
      const meta = global.mappingDb
        .prepare(
  ...
  try {
    ...
    try {
      ...
      providers = rows.filter(
        (r) => r.media_type.toLowerCase() === (type || "").toLowerCase(),
      );
    } catch (e) {}

    if (providers.length === 0) return null;

    const matched = providers.find((p) => name.includes(p.table_name));
    if (matched) {
      const keyCol = matched.primary_key_field;
      const row = global.mappingDb
        .prepare(
          `SELECT malid FROM ${matched.table_name} WHERE ${keyCol} = ? OR ${keyCol} LIKE ? LIMIT 1`,
        )
        .get(cleanId, `${cleanId}%`);
      if (row?.malid) return row.malid;
    }
    ...
    const sql = unionQueries.join(" UNION ") + " LIMIT 1";
    const params = providers.flatMap((_) => [cleanId, `${cleanId}%`]);

    const row = global.mappingDb.prepare(sql).get(...params);
      ...
      try {
        ...
        if (!malid) {
          ...
          if (!data.malid && FoundRow?.id) {
            const resolvedMalId = getMalIdFromMapping(
              type,
              FoundRow.provider,
              FoundRow.id,
            );
            if (resolvedMalId) {
              ...
              try {
                global.db
                  .prepare("UPDATE Anime SET MalID = ? WHERE id = ?")
                  .run(String(resolvedMalId), FoundRow.id);
              } catch (_) {}
            }
          }
        } else {
