...
local user_opts = {
    ...
    autoskip_intro = true,      -- automatically skip the intro segment (83s to 141s)
    autoplay_next = true,       -- automatically play the next file in the playlist when hitting the outro (1320s)
}
...
-- Stream Quality/Server selection menu variables
local server_menu = {
    active = false,
    selected_index = 1,
    sources = nil
...
local function get_stream_sources()
    ...
    local sources = {}
    for name, url in string.gmatch(sources_str, "([^|#]+)|([^#]+)") do
...
local function open_server_menu()
    ...
    -- Bind keys individually
    mp.add_forced_key_binding("UP", "server-menu-up", function()
        server_menu.selected_index = server_menu.selected_index - 1
        if server_menu.selected_index < 1 then
            server_menu.selected_index = #server_menu.sources
        end
        request_tick()
    end)
    
    mp.add_forced_key_binding("DOWN", "server-menu-down", function()
        server_menu.selected_index = server_menu.selected_index + 1
        if server_menu.selected_index > #server_menu.sources then
            server_menu.selected_index = 1
        end
        request_tick()
    end)
    
    mp.add_forced_key_binding("WHEEL_UP", "server-menu-wheel-up", function()
        server_menu.selected_index = server_menu.selected_index - 1
        if server_menu.selected_index < 1 then
            server_menu.selected_index = #server_menu.sources
        end
    ...
    mp.add_forced_key_binding("WHEEL_DOWN", "server-menu-wheel-down", function()
        server_menu.selected_index = server_menu.selected_index + 1
        if server_menu.selected_index > #server_menu.sources then
            server_menu.selected_index = 1
        end
        ...
        if (element.type == 'slider') then
            ...
            if element.name == "seekbar" then
                ...
                if dur > 0 then
                    ...
                    local function get_x_for_time(t)
                        local pct = t / dur
...
layouts = function ()
    ...
    add_area('input', get_hitbox_coords(posX, posY, 1, osc_geo.w, 120))
    
    -- area for top-left info button
    add_area('info_btn', get_hitbox_coords(0, 0, 7, 60, 48))
    
    -- area for skip intro button (at top right)
    add_area('skip_intro_btn', get_hitbox_coords(osc_geo.w - 110, 50, 5, 100, 32))
    ...
    lo.geometry = {x = refX , y = refY - 96 , an = 5, w = osc_geo.w - 50, h = 8}
    lo.layer = 13
    ...
    lo.geometry = {x = refX, y = refY - 96 , an = 5, w = osc_geo.w - 50, h = 22}
    ...
    lo = add_layout('skip_intro_bg')
    lo.geometry = {x = osc_geo.w - 110, y = 50, an = 5, w = 100, h = 32}
    lo.style = '{\\blur0\\bord0\\1c&H000000&\\1a&H40&}'
    lo.box.radius = 6
    lo.layer = 9
    lo.visible = true

    lo = add_layout('skip_intro')
    lo.geometry = {x = osc_geo.w - 110, y = 50, an = 5, w = 100, h = 32}
    lo.style = '{\\q2\\blur0\\bord0\\1c&HFFFFFF&\\fs18\\fn' .. (user_opts.font or "sans-serif") .. '}'
    lo.layer = 10
    lo.visible = true
...
function osc_init()
    ...
    -- Dynamic scaling for small window heights (tiling managers)
    if display_h > 0 and display_h < 720 then
        local scale_multiplier = 1.0 + (720 - display_h) / 470 * 0.6
        if scale_multiplier > 1.6 then scale_multiplier = 1.6 end
    ...
    -- skip_intro_bg
    ne = new_element('skip_intro_bg', 'box')
    ne.visible = true

    -- skip_intro
    ne = new_element('skip_intro', 'button')
...
function render()
    ...
    if osc_param.areas['info_btn'] then
        for _,cords in ipairs(osc_param.areas['info_btn']) do
            if state.osc_visible then
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, 'info_btn')
                mp.enable_key_bindings('info_btn')
            else
                mp.disable_key_bindings('info_btn')
            end
            if (mouse_hit_coords(cords.x1, cords.y1, cords.x2, cords.y2)) then
                mouse_over_osc = true
            end
    ...
    if osc_param.areas['skip_intro_btn'] then
        for _,cords in ipairs(osc_param.areas['skip_intro_btn']) do
            local skip_intro_el = elements['skip_intro']
            if skip_intro_el and skip_intro_el.enabled then
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, 'skip_intro_btn')
                mp.enable_key_bindings('skip_intro_btn')
            else
                mp.disable_key_bindings('skip_intro_btn')
            end
            if skip_intro_el and skip_intro_el.enabled and (mouse_hit_coords(cords.x1, cords.y1, cords.x2, cords.y2)) then
                mouse_over_osc = true
            end
        end
    end
...
function process_event(source, what)
    ...
    if not state.osc_visible and source == 'mbtn_left' then
        ...
        elseif what == 'up' then
            if state.active_element == 0 then
    ...
    if what == 'down' or what == 'press' then
        ...
        if not hit and source == 'mbtn_left' and what == 'down' then
            state.active_element = 0
        end
    ...
    elseif what == 'up' then
        if state.active_element == 0 then
            mp.command("cycle pause")
        elseif elements[state.active_element] then
...
set_virt_mouse_area(0, 0, 0, 0, 'info_btn')
set_virt_mouse_area(0, 0, 0, 0, 'skip_intro_btn')
