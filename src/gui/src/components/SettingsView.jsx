  ...
  return (
    <div className="settings-wrapper">
      ...
      <div className="settings-tabs-row">
        <button type="button" onClick={() => setActiveTab('general')} className={`settings-tab-btn ${activeTab === 'general' ? 'active' : ''}`}>General & UI</button>
        <button type="button" onClick={() => setActiveTab('anime')} className={`settings-tab-btn ${activeTab === 'anime' ? 'active' : ''}`}>Anime Settings</button>
        <button type="button" onClick={() => setActiveTab('manga')} className={`settings-tab-btn ${activeTab === 'manga' ? 'active' : ''}`}>Manga Settings</button>
        <button type="button" onClick={() => setActiveTab('mal')} className={`settings-tab-btn ${activeTab === 'mal' ? 'active' : ''}`}>MyAnimeList</button>
        <button type="button" onClick={() => setActiveTab('history')} className={`settings-tab-btn ${activeTab === 'history' ? 'active' : ''}`}>History & Stats</button>
      </div>
          ...
          <div className="settings-row">
            <div className="settings-panel glass-panel">
              ...
              <div className="settings-input-wrapper">
                ...
                <select value={discordRpc} onChange={(e) => setDiscordRpc(e.target.value)} className="settings-select">
            ...
            <div className="settings-panel glass-panel">
              ...
              <div className="settings-input-wrapper">
                ...
                <select value={pagination} onChange={(e) => setPagination(e.target.value)} className="settings-select">
        ...
        {activeTab === 'anime' && (
          <div className="settings-panel glass-panel">
            ...
            <div className="settings-input-wrapper">
              ...
              <select value={animeProvider} onChange={(e) => setAnimeProvider(e.target.value)} className="settings-select">
            ...
            <div className="settings-input-wrapper">
              <label className="settings-label">Preferred Streaming/Download Quality</label>
              <select value={quality} onChange={(e) => setQuality(e.target.value)} className="settings-select">
            ...
            <h3 className="settings-panel-title" style={{ marginBottom: '14px' }}>Subtitles Configuration</h3>
            <div className="settings-input-wrapper">
              <label className="settings-label">Merge Soft Subtitles into Video</label>
              <select value={mergeSubtitles} onChange={(e) => setMergeSubtitles(e.target.value)} className="settings-select">
                <option value="on">Yes (Merge subtitles inside MP4)</option>
                <option value="off">No (Download subtitles in subfolder)</option>
              </select>
              <span className="settings-hint">Merges multi-lingual soft subs directly into the video stream via FFmpeg.</span>
            </div>
            <div className="settings-input-wrapper">
              <label className="settings-label">Subtitle Format Conversion</label>
              <select value={subtitleFormat} onChange={(e) => setSubtitleFormat(e.target.value)} className="settings-select">
                <option value="srt">SubRip (.srt)</option>
                <option value="vtt">WebVTT (.vtt)</option>
              </select>
            </div>

            <button type="button" onClick={() => onMarketplaceOpen('Anime')} className="settings-market-btn" style={{ marginTop: '20px' }}>
        ...
        {activeTab === 'manga' && (
          <div className="settings-panel glass-panel">
            ...
            <div className="settings-input-wrapper">
              ...
              <select value={mangaProvider} onChange={(e) => setMangaProvider(e.target.value)} className="settings-select">
            ...
            <div className="settings-input-wrapper">
              ...
              <select value={autoLoadNextChapter} onChange={(e) => setAutoLoadNextChapter(e.target.value)} className="settings-select">
                <option value="on">Enabled</option>
                <option value="off">Disabled</option>
              </select>
            </div>
            <button type="button" onClick={() => onMarketplaceOpen('Manga')} className="settings-market-btn">
          ...
          <div className="settings-panel glass-panel">
            ...
            {malLoggedIn ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div className="settings-input-wrapper">
                  <label className="settings-label">Auto update anime status to:</label>
                  <select value={malStatus} onChange={(e) => setMalStatus(e.target.value)} className="settings-select">
                ...
                <button type="button" onClick={handleMalLogout} className="settings-logout-btn">
                  <LogOut size={16} />
              ...
              <>
                ...
                <div className="settings-stats-grid">
                  <div className="settings-stat-card glass-panel">
                    <span className="settings-stat-card-title">Total Watch Time</span>
                    <h3 className="settings-stat-card-val">{stats?.watchHours || 0} <span className="settings-stat-unit">hrs</span></h3>
                    <p className="settings-stat-card-sub">{stats?.completedEpisodes || 0} episodes completed ({stats?.distinctAnime || 0} titles)</p>
                  </div>
                  <div className="settings-stat-card glass-panel">
                    <span className="settings-stat-card-title">Total Read Time</span>
                    <h3 className="settings-stat-card-val">{stats?.readHours || 0} <span className="settings-stat-unit">hrs</span></h3>
                    <p className="settings-stat-card-sub">{stats?.completedChapters || 0} chapters completed ({stats?.distinctManga || 0} titles)</p>
                  </div>
                </div>

                {/* History Timeline */}
                <div className="settings-panel glass-panel">
                  <h2 className="settings-panel-title">Recent Activity History</h2>
                        ...
                        return (
                          <div key={idx} className="settings-history-item">
                            <div className="settings-history-item-left">
                              <span className={`settings-history-type-badge ${item.type === 'Anime' ? '' : 'manga'}`}>
                                {item.type}
                              </span>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                <strong className="settings-history-item-title">{item.title}</strong>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                              <div className="settings-history-item-right">
                                <span className="settings-history-item-date">{formattedDate}</span>
                                {item.is_completed === 1 && (
                                  <span className="settings-completed-badge">Completed</span>
                                )}
                              </div>
                              <button
                                type="button"
